...
from utils.inference_common import (
    ...
    read_labels_json, run_inference_on_image, run_detection_only, poly_iou, create_progress_bar
)
...
def parse_args():
    ...
    ap.add_argument("--det-only", action="store_true", help="Evaluate detection only (skip recognition)")
...
def match_detections_only(
    pred_polygons: List[List[List[int]]], gt_polygons: List[List[List[int]]]
) -> Tuple[List[Tuple[int,int,float]], List[int], List[int]]:
    """
    Detection only用のマッチング関数
    
    Args:
        pred_polygons: 予測されたpolygon座標のリスト
        gt_polygons: Ground truthのpolygon座標のリスト
    ...
    # GT ごとに最良 pred を割当（IoU 最大）
    for gi, gt_poly in enumerate(gt_polygons):
        best_pi, best_iou = -1, 0.0
        for pi, pred_poly in enumerate(pred_polygons):
            if pi in used_p:
                continue
            iou = poly_iou(pred_poly, gt_poly)
            if iou > best_iou:
                best_iou, best_pi = iou, pi
        if best_pi >= 0 and best_iou >= IOU_THRESH:
            matches.append((best_pi, gi, best_iou))
    ...
    fp = [i for i in range(len(pred_polygons)) if i not in used_p]
    fn = [j for j in range(len(gt_polygons)) if j not in used_g]
    return matches, fp, fn
...
def main():
    ...
    for fname, ann in create_progress_bar(labels.items(), desc="Evaluating images", total=len(labels)):
        ...
        for (pi, gi, iou) in matches:
            ...
            # Recognition評価はdet-onlyモードでは行わない
            if not getattr(args, 'det_only', False):
        ...
        # マッチしなかった分は E2E 観点では（det-onlyモードでは無視）：
        if not getattr(args, 'det_only', False):
            #   - 予測のみ（fp_idx）→ E2E FP
            #   - GTのみ（fn_idx）→ E2E FN
            e2e_fp += len(fp_idx)
    ...
    # Recognition 指標（マッチ成功ペアに対して、det-onlyモードでは無効）
    word_acc = (word_correct / word_total) if word_total > 0 else 0.0

    # End-to-End（Strict、det-onlyモードでは無効）
    ...
    # det-onlyモードでない場合のみrecognitionとend-to-endの結果を追加
    if not getattr(args, 'det_only', False):
        results["recognition"] = {
